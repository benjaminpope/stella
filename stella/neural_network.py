...
class ConvNN(object):
     ...
     def predict(self, modelname, times, fluxes, errs,
               multi_models=False, injected=False, verbose=True,
               progress: str = 'auto', window_batch: int = None):
          """
          ...
          show_outer = verbose and (progress in ('auto', 'lightcurves')) and (len(times) > 1)
          ...
          try:
               for j in range(len(times)):
                    ...
                    # Window progress: optionally predict in chunks and show tqdm over windows
                    if verbose and (progress == 'windows' or (progress == 'auto' and len(times) == 1)):
                         ...
                         wbar = tqdm(total=total_windows, desc='Windows', leave=False)
                         try:
          ...
          if injected:
               return self.predict_time, self.predict_flux, self.predict_err, self.predictions
